...
document.getElementById('diary-form').addEventListener('submit', (e) => {
    ...
    // 🌟 ১. Total Cost দিলে নগদ খরচ (Cash) অথবা বাকি খরচ (Due) ফিল্ড পূরণ করা বাধ্যতামূলক চেক
    if (totalCost > 0 && cashCost === 0 && dueCost === 0) {
        alert("মোট খরচ দিলে অবশ্যই নগদ খরচ অথবা বাকি খরচ পূরণ করতে হবে!");
        return;
    }

    // 🌟 ২. যদি বাকি খরচ (Due Cost) এ কোনো টাকা থাকে, তবে 'Due Cost Category' পূরণ করা বাধ্যতামূলক চেক
    if (dueCost > 0 && !dueCategory) {
        alert("বাকি খরচ থাকলে অবশ্যই 'Due Cost Category' পূরণ করতে হবে!");
        return;
    }

    // যোগফল ঠিক আছে কিনা চেক
    if ((cashCost + dueCost) !== totalCost) {
        alert("নগদ খরচ এবং বাকি খরচের যোগফল মোট খরচের সমান হতে হবে!");
        return;
    }
    ...
    .then(data => {
        ...
        if (data.success) {
            showToast("বাজার খরচ সফলভাবে ডাটাবেজে সেভ হয়েছে!");
            ...
            alert("ডেটা সেভ করতে সমস্যা হয়েছে: " + data.message);
        }
    })
    .catch(err => {
        submitBtn.innerText = "Save into Diary";
        ...
        alert("সার্ভার কানেকশন ব্যর্থ হয়েছে!");
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (payDueForm) {
        payDueForm.addEventListener('submit', (e) => {
            ...
            fetch(API + '/api/expenses/save-due-payment', { // এখানে রুট ঠিক করা হলো
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    if (payCookbillForm) {
        payCookbillForm.addEventListener('submit', (e) => {
            ...
            fetch(API + '/api/expenses/save-cook-payment', { // এখানে রুট ঠিক করা হলো
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function applyExpenseReadOnlyMode() {
    ...
    const uploadContainer = fileInput ? (fileInput.closest('label') || fileInput.parentElement) : null;
    if (uploadContainer) {
    ...
    // ৩. এক্সপেন্স পেজের অন্যান্য অ্যাকশন বাটনগুলো হাইড করা
    const payDueBtn = document.getElementById('pay-due-btn');
    ...
    // 🌟 ৪. মেম্বারদের জন্য 'Due Cost Category' এবং এর ইনপুট ফিল্ড হাইড করার লজিক
    const dueCategoryWrapper = document.getElementById('due-cost-category-wrapper');
    if (dueCategoryWrapper) {
        dueCategoryWrapper.style.display = 'none';
    }
}